...
<head>
  ...
  <title> Add Product | Shoppa </title>
    ...
    <div class="container">
        <form id="addNewProduct">
                ...
                <!-- Upload Image Section -->
                 <center class="" id="edit-info">
                   <div id="wrapper" class="img-container">
                    ...
                    <input type="file" onchange="preview_image(event)" style="display: none;" name="product-image" id="image-edit">
                   </div>
                   <br>
                   <span class="d-block" id="picture-name">Picture format: JPG, JPEG, PNG format only</span>
                 </center>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            ...
                            <input type="text" class="border w-100 p-2 bg-white text-capitalize" id="product-name" name="product-name" placeholder="Product Name Go There" required>
                            <h6 class="font-weight-bold pt-4 pb-1">Status:</h6>
                            <div class="row px-3">
                                <div class="col-lg-4 mr-lg-4 my-2 rounded bg-white">
                                    <input type="radio" name="active-selection" value="1" id="active" required checked>
                            ...
                            <textarea id="product-description" class="border p-3 w-100" rows="7" name="product-description" placeholder="Write details about your product" required></textarea>
                        </div>
                        <div class="col-lg-6">
                            <h6 class="font-weight-bold pt-4 pb-1">Select Category:</h6>
                            <select name="category-selection" id="category-selection" class="w-100" required>
                                <option value="0" selected disabled name="active-selection">Select Category</option>
                                <?php
                                  $result = $db->query($ALLCATEGORY);
                                  if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                      echo '
                                      <option value="'.$row["category_id"].'"name="active-selection">'.$row["category_name"].'</option>
                                      ';
                                    }
                                  }
                                ?>
                            ...
                            <div class="price">
                                ...
                                <div class="row px-3">
                                    <div class="col-lg-4 mr-lg-4 rounded bg-white my-2 ">
                                        <input type="number" min="0.00" max="100000.00" step="0.01" name="product-price" class="border-0 py-2 w-100 price" placeholder="0.00" id="product-price" required>
                                    </div>
                                </div>
                            </div>
            ...
            <div style="position:relative">
              ...
              <button type="submit" id="add-new-product" class="btn btn-primary d-block mt-2" style="position:absolute; left: 150px; bottom: 0.3px">Add New Product</button>
            </div>
...
$("document").ready(function(){
  $("#addNewProduct").on('submit',(function(e) {
    e.preventDefault();
    if(document.getElementById("image-edit").files.length == 0){
       document.getElementById("error-msg").innerHTML = "System Notice: Product Image has not yet uploaded.";
       return;
    } else {
      ...
      $.ajax({
          ...
          success: function(response) {
            if (response == 1) {
              alert("System Notice: New product added. Returning to Admin Panel...");
              location.href='admin_panel.php';
            } else {
              alert(response);
